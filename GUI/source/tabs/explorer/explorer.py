...
class Populater(QObject):
    ...
    def setModel(self, name, category, display, type, idx, allow_folder = True):
        ...
        q.prepare("INSERT OR REPLACE INTO models(name, category, display, type, file, folder, desc, idx, width, height, hash) VALUES (:name, :category, :display, :type, :file, :folder, :desc, :idx, :width, :height, :hash);")
        ...
        model_file_path = os.path.join(self.gui.modelDirectory(), name)
        file_hash = calculate_file_hash(model_file_path) if os.path.exists(model_file_path) else ""
        q.bindValue(":hash", file_hash)
...
class Explorer(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.conn = sql.Connection(self)
        self.conn.connect()
        self.conn.doQuery("CREATE TABLE models(name TEXT, category TEXT, display TEXT, type TEXT, file TEXT, folder TEXT, desc TEXT, idx INTEGER, width INTEGER, height INTEGER, hash TEXT, CONSTRAINT unq UNIQUE (category, idx));")
